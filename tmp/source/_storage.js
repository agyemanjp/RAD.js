import { ioFactoryForPostgres, ioFactoryForS3, ioFactoryForAPI, getStorageProviderFactory } from "@agyemanjp/storage";
import { dashCase, ok, snakeCase } from "@agyemanjp/standard";
export const getEntityStoreProviders = (cfg) => {
    const s3 = getStorageProviderFactory(ioFactoryForS3(cfg.s3Config), { cacheExpirySeconds: 0, transactionSupport: false });
    if (s3.isErr())
        return s3.errWithGenericValue();
    ...
    if (api.isErr())
        return api.errWithGenericValue();
    const entityStoreProviders = (entityName, entityXtendedName) => {
        return {
            dbReadWrite: pg.value(entityName)("get", "find", "insert", "update", "remove"),
            dbReadOnly: pg.value(entityXtendedName)("get", "find"),
            apiReadWrite: api.value(entityName)("get", "find", "insert", "update", "remove"),
            apiReadOnly: api.value(entityXtendedName)("get", "find")
        };
    };
