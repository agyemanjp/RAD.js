...
export function stdFieldUICtor(args: Pick<FieldUIArgs, | "title" | "labelPosition" | "style" | "labelStyle">) {
	...
	return (valueUIElt: UIElement, opts?: { titleCustom?: string, styleCustom?: CSSProperties }) => {
		const { titleCustom, styleCustom } = opts ?? {}
		const labelElt = labelPosition === "off"
			? <></>
			: <label /*htmlFor={id}*/ style={labelStyle}>{titleCustom ?? title}</label>
		return <StackPanel itemsAlignH="start" itemsAlignV="start"
			orientation={labelPosition === "left" ? "horizontal" : "vertical"}
			style={{ ...style, ...styleCustom ?? {} }}>

			{labelPosition === "bottom" ? [valueUIElt, labelElt] : [labelElt, valueUIElt]}
		</StackPanel>
	}
}
...
export type FieldUI<T = string> = (Component<{ value?: T; children?: never } & HtmlProps>)
